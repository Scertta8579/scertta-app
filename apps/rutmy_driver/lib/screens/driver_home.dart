...
import 'package:flutter_shared/services/rutmy_routing_service.dart';
...
import 'package:flutter_shared/widgets/panic_emergency_sheet.dart';
...
/// Fallback recto (mientras carga la ruta self-hosted o sin conexión).
...
class _DriverHomeScreenState extends State<DriverHomeScreen> with TickerProviderStateMixin {
  ...
  /// Polyline por calles (Valhalla) con viaje activo; null hasta cargar o sin viaje.
  ...
  Future<void> _cargarRutaParaViajeActivo(TripData viaje) async {
    final pts = await RutmyRoutingService.routeThrough(_tripWaypointsForRoute(viaje));
                  ...
                  child: Row(
                    children: [
                      SizedBox(
                        width: 48,
                        child: IconButton(
                          icon: const Icon(Icons.menu, color: Color(0xFF64DEB2)),
                          onPressed: () {
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: connected ? const Color(0xFF059669) : Colors.white,
            borderRadius: BorderRadius.circular(30),
            border: Border.all(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      '${filtrados.length}',
                      style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF64DEB2)),
                    ),
                  ),
                  IconButton(
                    icon: Icon(
...
class _TripRadarCardState extends State<_TripRadarCard> {
  ...
  void initState() {
    super.initState();
    _rutaMiniMapaFut = RutmyRoutingService.routeThrough(_tripWaypointsForRoute(widget.data));
  }
...
/// Mapa del diálogo "Ruta ampliada": polyline por calles (Valhalla).
class _MapaRutaAmpliada extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      width: double.infinity,
      child: FutureBuilder<List<LatLng>>(
        future: RutmyRoutingService.routeThrough(_tripWaypointsForRoute(viaje)),
        builder: (context, snap) {
