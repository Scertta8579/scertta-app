...
import 'package:flutter_shared/services/driver_session_service.dart';
    ...
    try {
      ...
      try {
        ...
        if (perfilResponse != null) {
          ...
          rolUsuario = null;
          print('━━━ PASO 4 COMPLETADO (sin perfil) ━━━');
          
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('⚠️ Sin perfil en base de datos. Completa el registro de conductor.'),
        ...
        rolUsuario = null;
        print('━━━ PASO 4 COMPLETADO (con error) ━━━');
        
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('⚠️ No se pudo leer el perfil. Intenta de nuevo o completa el registro.'),
              backgroundColor: Colors.orange,
      ...
      if (rolUsuario == 'conductor' || rolUsuario == 'ceo') {
        try {
          await DriverSessionService.registerDriverSession(supabase);
        } catch (e) {
          print('register_driver_session: $e');
        }
      }
      ...
      if (mounted) {
        ...
        switch (rolUsuario) {
          case 'conductor':
          case 'ceo':
            destinoScreen = const CriticalPermissionsGate(
              requireBackgroundLocation: true,
              child: DriverHomeScreen(),
            );
            nombreDestino = 'Scertta Conductor';
            print('📍 Destino: Scertta Conductor home (rol: $rolUsuario)');
            break;
          case 'solicitante':
            destinoScreen = const RegisterScreen();
            nombreDestino = 'Inscripción conductor';
            print('📍 Destino: formulario inscripción (rol: solicitante)');
            break;
          default:
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              const Text(
                'Esta aplicación es solo para Scertta Conductor.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 18,
