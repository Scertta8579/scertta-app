...
import RutmyEmpresaApiKey from "@/components/ceo/RutmyEmpresaApiKey";
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`relative inline-flex h-7 w-12 shrink-0 rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-rutmy-agua disabled:pointer-events-none ${
          checked ? "bg-rutmy-agua" : "bg-zinc-300 dark:bg-zinc-600"
        }`}
      >
        <span
...
export default function CeoConfiguracionNegocio() {
  ...
  const loadAiConfig = useCallback(async () => {
    setAiLoading(true);
    setAiError(null);
    // [NEUTRALIZADO] Tabla ai_automation_config no existe en DB (404)
    ...
    // }
    setAiEnabled(false);
    setAiLabel("Soporte y reclamos Nivel 1 (IA)");
    setAiLoading(false);
  }, []);
  ...
  // [NEUTRALIZADO] Realtime subscription a tabla inexistente
  // useEffect(() => {
  //   const ch = supabase
