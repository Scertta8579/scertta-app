...
// ═══════════════════════════════════════════════════════════════
// GET /api/gerencia/exportar
// Exporta reportes en CSV o JSON para el gerente (solo su franquicia)
//
// Query params:
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── Auth: solo gerente_franquicia ──
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return NextResponse.json({ error: "No autenticado" }, { status: 401 });

    const { data: gerente } = await supabase
      .from("perfiles")
      .select("rol, franquicia_id, franquicias(nombre)")
      .eq("id", user.id)
      .maybeSingle();

    if (!gerente || gerente.rol !== "gerente_franquicia") {
      return NextResponse.json({ error: "Acceso denegado. Solo gerente_franquicia." }, { status: 403 });
    }
    ...
    const franquiciaId = gerente.franquicia_id;
    const nombreFranquicia = (gerente as any).franquicias?.nombre || franquiciaId;
